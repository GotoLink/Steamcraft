...
public class ItemSCDrill extends ItemSCTool {
	...
	public boolean canHarvestBlock(Block block) {
		if (block == Steamcraft.oreQuartz || block == Steamcraft.oreQuartzActive || block == Steamcraft.oreVolucite || block == Steamcraft.oreUranite || block == Block.obsidian
				|| block == Block.oreGold || block == Block.oreLapis || block == Steamcraft.brimstone || block == Steamcraft.orePhosphate) {
			return false;
		}
		if (block == Block.blockIron || block == Block.oreIron || block == Block.blockLapis) {
			return toolMaterial.getHarvestLevel() >= 1;
		}
		if (block == Block.blockNetherQuartz || block == Steamcraft.decorBlock || block == Block.blockDiamond || block == Block.blockGold || block == Block.oreRedstone
				|| block == Block.oreRedstoneGlowing || block == Steamcraft.woodBrass) {
